...
def AddPrint(infile, outpath):
  ...
  if not outpath:
    if len(files) > 1:
      outpath = os.path.join(os.path.dirname(infile), "notext")
    else:
      outpath = os.path.splitext(infile)[0] + '_notext.pdf'
  ...
  if outisdir and os.path.samefile(os.path.dirname(infile), outpath):
    outpath = os.path.join(outpath, "notext")
    outisdir = os.path.isdir(outpath)
    outexists = os.path.exists(outpath)
...
def Main():
    ...
    if len(args) < 1:
        print("pdf-notext, remove all text objects from a PDF document")
        print("")
        parser.print_help()
        return
